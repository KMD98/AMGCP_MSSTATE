...
class NodeSubscriber:   
    def __init__(self):
        ...
        self.odometry_AMGCPdata = []
        self.odometry_dronedata = []
    ...
    def amgcp_callback(self, amgcp_data):
        ...
        if not self.processing:
            self.odometry_AMGCPdata.clear()
            amgcp_utm = utm.from_latlon(amgcp_data.amgcp_lat,amgcp_data.amgcp_lon) # returns(east,north,zone #, zone letter)
            self.odometry_AMGCPdata.append(amgcp_utm[0]) #easting,x
            self.odometry_AMGCPdata.append(amgcp_utm[1]) #northing,y
            self.odometry_AMGCPdata.append(amgcp_data.bearing)
            self.odometry_AMGCPdata.append(amgcp_data.speed2D)
    ...
    def drone_callback(self,drone_data):
        if not self.processing:
            self.odometry_dronedata.clear()
            drone_utm = utm.from_latlon(drone_data.drone_lat,drone_data.drone_lon) #returns(east,north,zone#, zone letter)
            self.odometry_dronedata.append(drone_utm[0]) #easting, x
            self.odometry_dronedata.append(drone_utm[1]) #northing, y
            self.odometry_dronedata.append(drone_data.drone_hmsl)
            self.new_message = True
