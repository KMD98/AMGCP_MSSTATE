...
#Remember to initialize gcp_xpath and gcp_ypath through interactive graph via MapChooseWP.py.
gcp_xpath = []
gcp_ypath = []
with open('gcp_xpath.txt','r') as fhandle:
    for x_coordinates in fhandle:
        gcp_xpath.append(float(x_coordinates))
with open('gcp_ypath.txt','r') as fhandle:
    for y_coordinates in fhandle:
        gcp_ypath.append(float(y_coordinates))
