...
void parsenRELPOSNED() {
  ...
  data[16] = tempHeading & 0xff;
  data[17] = tempHeading >> 8;
  data[18] = tempHeading >> 16;
  data[19] = tempHeading >> 24;
  // Turn heading double into a string
  Serial.print("Heading : ");Serial.println(((data[16]) + (data[17]<<8) + (data[18]<<16) + (data[19]<<24))/100.0f);
}
void requestEvent(){
  Wire2.write(data,20);
}
