...
from ros_essentials_cpp.msg import RTK_corrections,motor_rpm,operation_modes
import numpy as np
...
class localPlanner:
    ...
    def corrections_callback(self, message_gps):
        ...
        rospy.loginfo(self.goal_vector)
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            if self.first_time:
                self.node_init()
            if self.sw[2] and not self.first_time: #autonomous mode is selected. Keep in mind that the RTK is assumed available in the first iteration
