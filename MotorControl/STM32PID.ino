...
void loop() {
  int pot_valP = analogRead(POTP);
  delay(5);
  int pot_valD = analogRead(POTD);
  targetRPM[0] = map(pot_valD,0,1023,0,37); // Our max is 37rpm to be safe.
  targetRPM[1] = map(pot_valP,0,1023,0,37); // Our max is 37rpm to be safe.
  //Get rpm  
  ...
  call_PID(rpm[0],deltaTPID,targetRPM[0],1,0,0,PWM1,1);
  call_PID(rpm[1],deltaTPID,targetRPM[1],1,0,0,PWM2,1);
...
void call_PID(long rpm, float deltaTPID,long target,float kp,float ki,float kd,int pwmPin,int dir){
