...
with open('gcp_ypath.txt','r') as fhandle:
    for y_coordinates in fhandle:
        gcp_ypathWP.append(float(y_coordinates))
...
#We can make wp_number dynamic (user input) later on when we incorporated camera parameter. Uncomment two line below to use static y or longitude.
...
#Print inital gcp wp c = 0. C will get incremented in the loop
print("MGCP's first waypoint is:", gcp_x[c], gcp_y[c])
...
for i in range(len(x_coor)):
  ...
  if (displacementMeters > camera_intrack and (D2_y - prev_D2 > 0.0001) and (c < len(gcp_xpathWP)) and (x[i] > gcp_xpathWP[c])):
      ...
      print("Drone is heading away from MGCP and is at least", camera_intrack, "m from the MGCP")
      print("MGCP's new waypoint is:", gcp_x[c+1], gcp_y[c+1])
