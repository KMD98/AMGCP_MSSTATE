...
class BrushedDCPID{
  ...
  void evaluate(int rpm, int target, float deltaT, int &duty, int &dir){
    ...
    dir = 1;
    duty = (int) fabs(u); //We grab the magnitude of u because signs only show direction of voltage. Direction will be determined by high level controller.
    if(u >= umax){
      duty = umax;
    }
    else if ( u <= 0){
...
void loop() {
  ...
  int target = map(pot_val,0,1023,0,37); // Our max is 37rpm to be safe.
  long rpm[] = {0,0,0,0};
  //Get rpm
  int pos[] = {0,0,0,0};
  //Determine rpm for all 4 motors and set current to previous for next iteration.
  for (int enc_num = 0; enc_num < sizeof(pos); enc_num++){
  ...
  for(int k = 0;k < sizeof(NMOTORS);k++){
    ...
    pid[k].evaluate(rpm[k],target,deltaTPID,duty,dir);
    // signal the motor
